...
<?php
    ...
    $sql = "SELECT * FROM pedido group by cpf, data order by data desc";
    $res = $conn->query($sql);
    $qtd = $res->num_rows;

    print"<button onclick=\"location.href='?page=cadastrarPedido'\" class='btn btn-success mb-3'>Cadastrar Pedido <i class='fa-solid fa-plus'></i></button>";
    ...
    if($qtd > 0) {
        ...
        while($row = $res->fetch_object()) {
            ...
            <button onclick=\"location.href='?page=editarPedido&id=".$row->id."'\" class='btn btn-primary mb-1' >Editar <i class='fa-solid fa-pen-to-square'></i></button>
            ...
            class='btn btn-danger mb-1'>Excluir <i class='fa-solid fa-trash'></i></button>
            <button onclick=\"location.href='?page=desconto&id=".$row->id."'\" class='btn mb-1 btn-warning'>Desconto <i class='fa-solid fa-percent'></i></button>
            </td>";
            print "</tr>";
        }
        print"</table>";

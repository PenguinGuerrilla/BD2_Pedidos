...
switch ($_REQUEST["acao"]) {
    ...
    case "desconto":
        ...
        try {
            $sql = "call desconto({$id_pedido},{$desconto})" ;
            $res = $conn->query($sql);

            if ($res == true) {
                print "<script>alert('Desconto efetuado com sucesso!');</script>";
                print "<script>location.href='?page=listarPedido';</script>";
            }
        } catch (mysqli_sql_exception $e) {
            //die('Erro: '.$e->getMessage());
            print "<script>alert('Não foi possível efetuar o desconto.');</script>";
            print "<script>location.href='?page=listarPedido';</script>";
        }
